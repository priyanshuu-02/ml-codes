...
def predict(model, normalizer, imu, initial_speed, device):
    ...
    orientation = (output["heading_delta"].cpu().numpy() if "heading_delta" in output
                   else normalizer.inverse_yaw(output["yaw_rate"].cpu().numpy()))
...
def predict_many(model, normalizer, imu, initial_speed, device, batch_size=512):
    ...
    with torch.no_grad():
        for start in range(0, len(x), batch_size):
            ...
            yaws.append((output["heading_delta"] if "heading_delta" in output else output["yaw_rate"]).cpu().numpy())
            motions.append(output["motion_logits"].argmax(dim=1).cpu().numpy())
    orientation = np.concatenate(yaws)
    if "heading_delta" not in output:
        orientation = normalizer.inverse_yaw(orientation)
    return normalizer.inverse_speed(np.concatenate(speeds)), normalizer.inverse_position(np.concatenate(positions)), orientation, np.concatenate(motions)


def closed_loop_metrics(model, normalizer, imu, targets, device, horizon_seconds, orientation_is_delta=False, window_size=20):
    ...
    for step in range(steps):
        ...
        ph += np.degrees(yaw if orientation_is_delta else yaw * duration); ps = np.maximum(0.0, pred_speed)
...
def main():
    ...
    parser.add_argument("--model-version", choices=("v7", "v8"), default="v7")
    ...
    model_class = V8DeadReckoningModel if args.model_version == "v8" else V7DeadReckoningModel
    model = model_class(input_channels=6, conv_dim=96, hidden_dim=128, dropout=0.15).to(device)
    ...
    orientation_target = "heading_delta" if args.model_version == "v8" else "yaw_rate"
    all_windows = {key: np.concatenate([w[key] for w in truth]) for key in ("speed", orientation_target, "motion")}
    ...
    result = {"checkpoint": str(checkpoint_path), "test_set_used_for_selection": False,
              ...
              ("heading_delta_mae_rad" if args.model_version == "v8" else "yaw_mae_rad_s"): float(np.abs(all_pred_yaw - all_windows[orientation_target]).mean()),
    ...
    if not args.skip_trajectory:
        for horizon in (10, 20, 30, 60):
            by_session = {s["session_id"]: closed_loop_metrics(model, normalizer, *prepare_session(dataset, s), device, horizon, orientation_is_delta=args.model_version == "v8") for s in sessions}
